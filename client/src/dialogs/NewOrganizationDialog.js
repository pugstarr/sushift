...
const NewOrganizationDialog = ({ onClose, onCreate }) => {
  ...
  const handleCreate = async (event) => {
    ...
    try {
        ...
        const response = await axios.post(`${API_URL}/orgs/create`, { name: orgName, userId });
  ...
  const handleJoin = async(event) => {
    ...
    try{
      const response = await axios.post(`http://localhost:8000/orgs/addUser`, {userId: userId, joinCode : link});
      onClose();
    }
    catch(error) {
